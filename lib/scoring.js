...
export function scoreHolders(rugData, holderData) {
  ...
  const totalHolders = rugData.holderCount || topHolders.length;
...
export function buildAiPromptData(pair, rugData, scores, holderData) {
  ...
  if (rugData) {
    ...
    const top10Pct = topHolders
      .slice(0, 10)
      .reduce((sum, h) => sum + (h.pct || 0), 0);
    lines.push(`Top 10 Holder Concentration: ${top10Pct.toFixed(1)}%`);
    lines.push(
      `Total Holders: ${rugData.holderCount || topHolders.length || "unknown"}`
    );
  }

  // On-chain holder analysis
  if (holderData) {
    lines.push(`\n--- On-Chain Holder Analysis (Top 20 via RPC) ---`);
    lines.push(`Total Supply: ${holderData.totalSupply?.toLocaleString()}`);
    lines.push(`Top 5 Holders Control: ${holderData.top5Pct}%`);
    lines.push(`Top 10 Holders Control: ${holderData.top10Pct}%`);
    lines.push(`Top 20 Holders Control: ${holderData.top20Pct}%`);
    lines.push(`Largest Single Holder: ${holderData.topHolderPct}%`);
    lines.push(`Skew Ratio (#1 vs rest of top 10): ${holderData.skewRatio}`);
    lines.push(
      `Distribution: ${holderData.tiers.whales} whales (>2%), ${holderData.tiers.dolphins} dolphins (0.5-2%), ${holderData.tiers.fish} fish (0.1-0.5%), ${holderData.tiers.shrimp} shrimp (<0.1%)`
    );
  }
